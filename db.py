...
feeds_table = Table ('feeds', metadata,
    ...
    CheckConstraint ('title_value IS NULL = title_type IS NULL', name = 'feeds_title_has_type'),
    CheckConstraint ("title_value != ''", name = 'feeds_title_not_empty'),
    Column ('link', Unicode),
...
entries_table = Table ('entries', metadata,
    ...
    UniqueConstraint ('guid', 'feed_id', name = 'entries_unique_guid_feed'),
    Column ('title_value', Unicode),
    Column ('title_type', String),
    CheckConstraint ('title_value IS NULL = title_type IS NULL', name = 'entries_title_has_type'),
    CheckConstraint ("title_value != ''", name = 'entries_title_not_empty'),
    Column ('link', Unicode),
    Column ('content_value', Unicode),
    Column ('content_type', String),
    CheckConstraint ('content_value IS NULL = content_type IS NULL', name = 'entries_content_has_type'),
    CheckConstraint ("content_value != ''", name = 'entries_content_not_empty'),
    Column ('summary_value', Unicode),
    Column ('summary_type', String),
    CheckConstraint ('summary_value IS NULL = summary_type IS NULL', name = 'entries_summary_has_type'),
    CheckConstraint ("summary_value != ''", name = 'entries_summary_not_empty'),
...
class MaybeHTML (object):
    ...
    def as_html (self):
        ...
        if self.__content_type == 'text/plain':
            ...
            return cgi.escape (self.__data).replace ('\n', '<br>')
        if self.__content_type == 'text/html':
